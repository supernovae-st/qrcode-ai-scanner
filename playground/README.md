...
engine · symbology), and the Vite `.wasm` resolution gotcha (`optimizeDeps.exclude`).
...
Chrome and asserts **every feature** — all 5 samples (symbology · payload · score
· axes · UEC · ISO card · hints · rescue · GS1 conformance), profile switching,
the upload path, the FNC1 control-byte glyph, and live `scan_frame` on a fake
device. Exits non-zero on any regression; writes screenshots to `/tmp/pg-*.png`.

> When `@supernovae-st/qrcode-ai-scanner-wasm@0.3.0` is published to npm, swap
> the `file:` dependency for the real version — nothing else changes.
